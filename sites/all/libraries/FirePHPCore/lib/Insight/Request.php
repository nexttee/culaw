...
class Insight_Request
{
    ...
    public function cachePathForName($name, $type)
    {
        ...
        if (strpos(array_pop($nameParts), '.') > 1 || substr($name, -1,1) === '/') {
            return $this->_normalizePath($file . DIRECTORY_SEPARATOR . $name);
        } else {
            return $this->_normalizePath($file . DIRECTORY_SEPARATOR . $name . '.json');
        }
    }
    ...
    private function _normalizePath($path)
    {
        ...
        for ($i = 0; $i < sizeof($oldPath); $i++)
        {
            if ($oldPath[$i] === '.' || !sizeof($oldPath[$i]))
                continue;
            if ($oldPath[$i] === '..')
            {
                if (!sizeof($newPath))
